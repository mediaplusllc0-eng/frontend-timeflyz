...
export default function Navbar({
  ...
  const handleKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      e.preventDefault();
      if (localSearchValue) {
        router.push(`/slots?city=${encodeURIComponent(localSearchValue)}`);
      }
    }
  };
  ...
  return (
    <>
                  ...
                  className={`uppercase ${
                    isFixed
                      ? scrolled
                        ...
                        : textColor
                      : "text-white font-medium"
                  } text-sm md:text-sm lg:text-lg tracking-wide ${hoverColor} transition font-lora`}
                >
                  {item.title}
                </Link>
              </div>
      ...
      <Model isOpen={isOtpModalOpen} onClose={() => setIsOtpModalOpen(false)}>
        <OtpVerification
